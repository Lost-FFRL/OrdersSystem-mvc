$(function() {
	...
	$("#p-add").click(function() {
		...
		if (exist) {
			...
			var html = "<tr id='"
					...
					+ "<input type='hidden' name='opId' value='0' />"
	...
	function del(self){
		...
		self.attr({"flag":1});
		// 获取商品
		$.post("orders.do?type=delOP",{
			id: self.attr("opId")
		}, function(data) {
			if (data && data.type == 0){
				$("#del_prompt").text("删除成功").show().fadeOut(5000);
				self.attr({"flag":0});
				self.parent().parent().remove();
			} else {
				$("#del_prompt").text("删除失败").show().fadeOut(5000);
				self.attr({"flag":0});
			}
		}, "JSON");
	}
			...
			for ( var obj in data) {
				if (data[obj]) {
					html += "<option value='" + data[obj].id + "' p-name='"
							...
							+ "' p-desc='" + data[obj].desc + "' p-num='"
							+ data[obj].number + "' >" + data[obj].name
							+ "</option>";
				}
			}
